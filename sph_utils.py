...
def face_to_cube_coord(face_gr, x, y, z):
    ...
    # front/back
    fb = (face_gr == FACE_F)
    direct[fb, 0] =  z[fb]; direct[fb, 1] =  y[fb]; direct[fb, 2] =  x[fb]
    bb = (face_gr == FACE_B)
    direct[bb, 0] = -z[bb]; direct[bb, 1] =  y[bb]; direct[bb, 2] =  x[bb]

    # top/bottom
    tb = (face_gr == FACE_T)
    direct[tb, 0] =  z[tb]; direct[tb, 1] = -x[tb]; direct[tb, 2] =  y[tb]
    db = (face_gr == FACE_D)
    direct[db, 0] =  z[db]; direct[db, 1] =  x[db]; direct[db, 2] =  y[db]

    # right/left
    rb = (face_gr == FACE_R)
    direct[rb, 0] = -x[rb]; direct[rb, 1] =  y[rb]; direct[rb, 2] =  z[rb]
    lb = (face_gr == FACE_L)
    direct[lb, 0] =  x[lb]; direct[lb, 1] =  y[lb]; direct[lb, 2] =  z[lb]
